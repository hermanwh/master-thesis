...
def main(filename, targetColumns):
    ...
    train_metrics_lstm = utilities.calculateMetrics(y_train[ENROL_WINDOW:], lstmPredictions)
    test_metrics_lstm = utilities.calculateMetrics(y_test[ENROL_WINDOW:], lstm_test)

    print(train_metrics_lstm)
    print(test_metrics_lstm)


    train_metrics_mlp = utilities.calculateMetrics(y_train[ENROL_WINDOW:], mlpPredictions)
    test_metrics_mlp = utilities.calculateMetrics(y_test[ENROL_WINDOW:], mlp_test)
    ...
    pred_train = model.predict(predictions)
    pred_test = model.predict(asd)
